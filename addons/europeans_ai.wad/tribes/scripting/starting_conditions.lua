...
function set_starting_warecount(player)
    ...
    for i, worker in ipairs(tribe.workers) do
        ...
        elseif string.find(worker.name, "basic") then
            for j, warehouse in ipairs(warehouses) do
                warehouse:set_workers(worker.name, math.ceil((worker_economy:target_quantity(worker.name) + 12) / #warehouses))
            end
    ...
    for i, ware in ipairs(tribe.wares) do
        if (ware:is_construction_material(player.tribe_name)) then
            for j, warehouse in ipairs(warehouses) do
                warehouse:set_wares(ware.name, math.ceil((ware_economy:target_quantity(ware.name) + 64) / #warehouses))
            end
        else
            for j, warehouse in ipairs(warehouses) do
...
function allow_warehouses_per_ware_amount(player)
    ...
    if (enough_ware_amount) and ((map.allows_seafaring == true) and (map.number_of_port_spaces > 0)) then
    ...
    for i, slot in ipairs(map.player_slots) do
        ...
        if (sf.owner == player) then
            if not (sf.immovable) or ((sf.immovable) and not ((sf.immovable.descr.type_name == "constructionsite") or (sf.immovable.descr.type_name == "warehouse"))) then
                player:place_building("europeans_warehouse", sf, true, true)
                break
            end
        end
    end
...
function doing_ai_stuff(player, increment, timefactor)
    ...
    if (increment >= timefactor * 2) then
