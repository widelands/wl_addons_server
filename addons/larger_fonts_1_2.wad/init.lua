...
local fs_font_size = 20

local wui_font_color = {255, 255, 0}
local wui_font_face = "sans"
local wui_font_size = 20

local fs_text_padding = 9
...
local h1_font_size = 27
local h2_font_size = 23
local h3_font_size = 20
local h4_font_size = 18
local p_font_size = 18

local unknown_font_color = {209, 45, 45}
local unknown_font_face = "serif"
local unknown_font_size = 25
...
local rv = {
   ...
   -- Windows
   -- Border definition changed in v1.3, version dependent entries will be added
   -- in a separate step at the end.
   -- This is not strictly necessary, because this section is optional for add-ons
   -- since v1.2, but unfortunately font entries must include the color, so we can
   -- only ensure a consistent color scheme if we include everything.
   ...
   paragraphs = {
      ...
      wui_lore_author = {
         ...
         align = "right",
         space_before = wui_text_space_before,
         space_after = wui_text_space_after + 5,
      },
   ...
   styled_sizes = {
      ...
      fs_text_space_before_inline_header = 12,
      fs_main_menu_dropdown_height = 33,
      wui_text_default_gap = wui_text_padding,
      wui_text_space_before_inline_header = 12,
      wui_space_before_immovable_icon = 20,
      win_condition_message_gap = 12,
      help_terrain_tree_header_space_before = 18,
      help_terrain_tree_header_space_after = 5,
      editor_tooltip_icon_gap = 3,
      campaign_message_box_default_h = 500,
      campaign_message_box_default_w = 600,
      campaign_message_box_top_pos_y = 25,
      campaign_message_box_size_step = 70,
      campaign_message_box_h_min = 200,
      campaign_message_box_h_max = 600,
      campaign_message_box_w_min = 300,
      campaign_message_box_w_max = 700,
      campaign_fri02_poem_indent = 16,
...
if string.len(wlver) < 3 then
   error(string.bformat("Error parsing version string: %1$s -> %2$s", get_build_id(), wlver))
end

if string.len(wlver) == 3 and wlver + 0.0 < 1.3 then
   rv.windows = {
      fsmenu = {
         -- red, green, blue, alpha
