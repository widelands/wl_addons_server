...
public class Buildcats {
	...
	private static void doRunXgettext(String file, String out) throws Exception {
		doRunXgettext(file, new FileInputStream(file), out);
	}

	private static void doRunXgettext(String filename, InputStream fileContent, String out)
	    throws Exception {
		final boolean needsProcessing = !filename.toLowerCase().endsWith("lua");
		Utils.Profile profile = null;
		if (needsProcessing) profile = Utils.readProfile(fileContent, filename);
		...
		PrintStream print = needsProcessing ? new PrintStream(pipe) : null;
		...
		for (File f : Utils.listSorted(dir)) {
			if (f.isDirectory()) {
				...
				for (Enumeration e = zip.entries(); e.hasMoreElements();) {
					...
					doRunXgettext(entry.getName(), zip.getInputStream(entry), out);
				}
			} else if (!mapsOnly && isXGettextInput(f.getName())) {
				doRunXgettext(f.getPath(), out);
			}
		}
	}
	...
	public static void buildCatalogues() throws Exception {
		...
		doRunXgettext(uploaderCommentsFile.getAbsolutePath(), out);
