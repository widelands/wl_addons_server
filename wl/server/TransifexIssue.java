...
public class TransifexIssue {
	...
	private TransifexIssue(String issueID, String message, String string, String stringID, String occurrence) {
	...
	public static List<TransifexIssue> checkIssues() throws Exception {
		...
		Map json = (Map)parser.parse(Utils.bashOutput("curl", "-g",
				"-H", "Authorization: Bearer " + Utils.config("transifextoken"),
		...
		for (Object oneIssue : (List)json.get("data")) {
			String sourceStringURL = ((Map)((Map)((Map)((Map)oneIssue).get("relationships")).get("resource_string")).get("links")).get("related").toString();
			Map sourceStringQuery = (Map)parser.parse(Utils.bashOutput(
					"curl", "-g", "-H", "Authorization: Bearer " + Utils.config("transifextoken"), sourceStringURL), cf);
			...
			result.add(new TransifexIssue(
					...
					((Map)((Map)sourceStringQuery.get("data")).get("attributes")).get("appearance_order").toString(),
					((Map)((Map)sourceStringQuery.get("data")).get("attributes")).get("occurrences").toString()
			));
		}
		return result;
	}
