		...
		do {
			try {
				...
				if (phase == 0) {
					...
					ResultSet sql = Utils.sql(Utils.Databases.kAddOns, "select name from addons where security=0 order by name");
					while (sql.next()) unverified.add(sql.getString("name"));
					sql = Utils.sql(Utils.Databases.kAddOns, "select name from addons where security>0 and quality=0 order by name");
					while (sql.next()) unassessed.add(sql.getString("name"));
					sql = Utils.sql(Utils.Databases.kAddOns, "select name from addons where security>0 and quality>0 order by name");
					while (sql.next()) {
						String name = sql.getString("name");
						if (Utils.bashResult("tx", "status", "-r", ServerUtils.toTransifexResource(name)) != 0) notx.add(name);
					}
					if (!unverified.isEmpty() || !unassessed.isEmpty() || !notx.isEmpty()) {
						String message = String.format("There are currently %d unverified add-ons, %d add-on awaiting quality review, and %d add-ons without Transifex integration.", unverified.size(), unassessed.size(), notx.size());
						...
						Utils.sendEMailToSubscribedAdmins(Utils.kEMailVerbosityFYI, "Moderation Report", message);
					}
