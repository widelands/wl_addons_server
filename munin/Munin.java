...
public class Munin {
	...
	private static final MultiGraph[] _graphs = new MultiGraph[] {
		new MultiGraph("addons_uptime", "Add-Ons Server Uptime", "time",
			new SubGraph("time", "Uptime in ms", "Time in milliseconds since the last server restart", "GAUGE")
		),
		new MultiGraph("addons_users", "Add-Ons Server Users", "count",
			new SubGraph("registered", "Registered", "Number of currently connected registered users", "GAUGE"),
			new SubGraph("unregistered", "Unregistered", "Number of currently connected unregistered users", "GAUGE"),
		...
		new MultiGraph("addons_commands", "Add-Ons Commands", "count",
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (String[] s : output) for (int i = 0; i < s.length; i++) s[i] = in.readLine();
		} catch (Exception e) {
			System.err.println(e);
		} finally {
			for (int i = 0; i < output.length; i++) {
				System.out.println("multigraph " + _graphs[i].name);
				for (int j = 0; j < output[i].length; j++) System.out.println(_graphs[i].subgraphs[j].name + ".value " + output[i][j]);
			}
		}
	}
}
